...
// Allow providing contract address via file (useful in containerized deployer flows)
const defaultContractFile = '/workspace/hardhat/contract-address.txt';
const defaultPackFile = '/workspace/hardhat/pack-address.txt';
if (!process.env.NFT_CONTRACT_ADDRESS_FILE) {
    process.env.NFT_CONTRACT_ADDRESS_FILE = defaultContractFile;
}
if (!process.env.PACK_CONTRACT_ADDRESS_FILE) {
    process.env.PACK_CONTRACT_ADDRESS_FILE = defaultPackFile;
}
if (!process.env.NFT_CONTRACT_ADDRESS && process.env.NFT_CONTRACT_ADDRESS_FILE) {
    try {
        const fs = await import('node:fs/promises');
        const addr = (await fs.readFile(process.env.NFT_CONTRACT_ADDRESS_FILE, 'utf8')).trim();
        if (addr) {
            process.env.NFT_CONTRACT_ADDRESS = addr;
        }
    } catch {
    }
}
if (!process.env.PACK_CONTRACT_ADDRESS && process.env.PACK_CONTRACT_ADDRESS_FILE) {
    try {
        const fs = await import('node:fs/promises');
        const addr = (await fs.readFile(process.env.PACK_CONTRACT_ADDRESS_FILE, 'utf8')).trim();
        if (addr) {
            process.env.PACK_CONTRACT_ADDRESS = addr;
        }
    } catch {
    }
}
