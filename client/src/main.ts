...
function openShop() {
    const existing = document.querySelector("shop-modal");
    if (existing) return;
    const modal = document.createElement("shop-modal");
    modal.addEventListener("nft:updated", () => {
        document.dispatchEvent(new CustomEvent("collection:refresh"));
    });
    document.body.appendChild(modal);
}
