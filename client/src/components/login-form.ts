...
`;
    ...
    private async handleWeb3Login(msgEl: HTMLElement) {
        const API = (window as any).__CFG__.API_URL;
        const ethereum = (window as any).ethereum;
        msgEl.textContent = "";

        if (!ethereum || !ethereum.request) {
            msgEl.textContent = "No Web3 wallet detected.";
            msgEl.className = "text-sm text-red-600 mt-1 mb-1";
            return;
        }
        ...
        try {
            ...
            if (!wallet) {
                msgEl.textContent = "Wallet connection failed.";
                msgEl.className = "text-sm text-red-600 mt-1 mb-1";
                return;
            }

            const challengeRes = await fetch(`${API}/login/siwe/challenge`, {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({walletAddress: wallet}),
            });
            const challenge = await challengeRes.json().catch(() => null);
            if (!challengeRes.ok || !challenge?.ok || !challenge?.message || !challenge?.nonce) {
                msgEl.textContent = "Unable to start Web3 login.";
                msgEl.className = "text-sm text-red-600 mt-1 mb-1";
                return;
            }
            ...
            const verifyRes = await fetch(`${API}/login/siwe/verify`, {
                method: "POST",
                headers: {"Content-Type": "application/json"},
                body: JSON.stringify({walletAddress: wallet, signature, nonce: challenge.nonce}),
            });
            const verify = await verifyRes.json().catch(() => null);

            if (verifyRes.ok && verify?.ok && verify?.token) {
                setToken(verify.token);
                msgEl.textContent = "Logged in with Web3.";
                msgEl.className = "text-sm text-green-700 mt-1 mb-1";
                setTimeout(() => {
                    this.dispatchEvent(new CustomEvent("login:success", {
                        detail: {user: verify.user ?? {username: wallet}, token: verify.token},
                        bubbles: true
                    }));
                }, 300);
                return;
            ...
            msgEl.textContent = "Web3 login failed.";
            msgEl.className = "text-sm text-red-600 mt-1 mb-1";
        } catch (err: any) {
            msgEl.textContent = err?.message || "Web3 login failed.";
            msgEl.className = "text-sm text-red-600 mt-1 mb-1";
        }
    }
});
